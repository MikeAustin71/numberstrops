...
type numStrFmtSpecLibQuark struct {
	lock *sync.Mutex
}
...
// is set to 'false'.
//
func (nStrFmtSpecLibQuark *numStrFmtSpecLibQuark) testValidityOfNumSepsDto(
	nStrFmtSpecDigitsSepDto *NumStrFmtSpecDigitsSeparatorsDto,
	ePrefix string) (
	isValid bool,
	err error) {

	if nStrFmtSpecLibQuark.lock == nil {
		nStrFmtSpecLibQuark.lock = new(sync.Mutex)
	}

	nStrFmtSpecLibQuark.lock.Lock()

	defer nStrFmtSpecLibQuark.lock.Unlock()

	ePrefix += "numStrFmtSpecLibQuark.testValidityOfNumSepsDto() "
